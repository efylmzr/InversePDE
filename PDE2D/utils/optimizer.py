...
class Optimizer:
    ...
    def set_learning_rate(self, lr) -> None:
        ...
        if isinstance(lr, float) or isinstance(lr, int):
            ...
            self.lr_default_v = dr.opaque(dr.detached_t(TensorXf), lr, shape=())
        elif isinstance(lr, dict):
            for k, v in lr.items():
                self.lr[k] = v
                #self.lr_v[k] = dr.opaque(dr.detached_t(Float), v, shape=1)
                self.lr_v[k] = dr.opaque(dr.detached_t(TensorXf), v, shape=())
        else:
...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        for k, p in self.variables.items():
            ...
            lr_scale = dr.opaque(dr.detached_t(TensorXf), lr_scale, shape=())
