...
wos = WostVariable(data_holder, green_sampling=green, use_accelaration=use_accel, opt_params= ["screening.texture.tensor"])
...
for i in range(image.shape[0]):
    for j in range(image.shape[1]):
        σ1, σ2 = step_texture(σ, i, j, fd_step)
        data_holder1 = DataHolder(shape = shape, α = α, α_split = α, σ = σ1, σ_split = σ, f = f)
        data_holder2 = DataHolder(shape = shape, α = α, α_split = α, σ = σ2, σ_split = σ, f = f)
...
def create_path(path):
    if not os.path.exists(path):
        os.makedirs(path)
path = os.path.join(PATH, "output2D", "finite_differences", "variable", "screening")
create_path(path)

fig.savefig(os.path.join(path, f"screening{res_tensor}-fd{fd_step}.pdf"), bbox_inches='tight', pad_inches=0.04, dpi=200)
record = {}
record["prb"] = grad_prb
record["fd"] = fd_grad
np.save(os.path.join(path, f"screening{res_tensor}-fd{fd_step}.npy"), record)